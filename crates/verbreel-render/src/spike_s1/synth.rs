//! Synthetic raw YUV420P frame generator (slice B copy).
//!
//! Manually synced from `crates/verbreel-codec-native/src/spike_s1/synth.rs`.
//! If you change one, change both — both are throwaway spike code; sharing
//! across crates for two ~100-line files would cost more than it saves
//! (would need a new `verbreel-spike-fixtures` crate just for this).
//!
...
//! integer in the top-left 4-byte cell. The encoded counter ensures
//! every frame differs from its neighbors.
//!
...
/// # Panics
///
/// Panics if `width` or `height` is not divisible by 2, or if the
