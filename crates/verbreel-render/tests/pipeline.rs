...
fn run_returns_not_yet_implemented() {
    ...
    let err = run(&mut p, 0).expect_err("v0 must always Err");
...
fn run_detail_is_non_empty() {
    let mut p = Pipeline::new(RenderPreset::Deterministic);
    let RenderError::NotYetImplemented { detail } = run(&mut p, 0).unwrap_err() else {
...
fn run_with_performance_preset_also_errs() {
    ...
    let err = run(&mut p, 0).expect_err("v0 must Err");
    assert!(matches!(err, RenderError::NotYetImplemented { .. }));
}
...
fn run_with_negative_tk_does_not_branch_on_sign() {
    let mut p = Pipeline::new(RenderPreset::Deterministic);
    let err = run(&mut p, -1).expect_err("v0 must Err");
    assert!(matches!(err, RenderError::NotYetImplemented { .. }));
}
...
fn run_with_max_tk_does_not_overflow() {
    let mut p = Pipeline::new(RenderPreset::Deterministic);
    let err = run(&mut p, i64::MAX).expect_err("v0 must Err");
    assert!(matches!(err, RenderError::NotYetImplemented { .. }));
}
...
fn run_called_twice_consistent() {
    let mut p = Pipeline::new(RenderPreset::Deterministic);
    let first = run(&mut p, 0).expect_err("v0 must Err");
    let second = run(&mut p, 0).expect_err("v0 must Err");
