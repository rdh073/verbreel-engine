//! Tests for `timeline.redo` (§12.4) — v1 redo-stack floor.

use std::sync::Arc;

use serde_json::{Value, json};
use verbreel_state::verbs::timeline_redo::{compute_patch, resolved_steps};
use verbreel_state::{
    MutateOutcome, Project, TimelineRedoArgs, TimelineRedoData, TimelineRedoError,
    TimelineRedoVerb, Verb, VerbError, VerbRegistry, default_fixtures, default_registry,
...
fn args_default() -> TimelineRedoArgs {
    TimelineRedoArgs {
...
fn args_unknown_field_rejected_by_deny_unknown_fields() {
    let err = serde_json::from_value::<TimelineRedoArgs>(json!({
        "project_id": FIXTURE_PROJECT_ID,
        "steps": 1,
        "extra": true,
    }))
...
fn compute_patch_zero_steps_is_schema_violation() {
    ...
    let TimelineRedoError::SchemaViolation { detail } = err else {
...
#[test]
fn timeline_redo_error_into_verb_error_bad_args_for_schema_violation() {
    let err = TimelineRedoError::SchemaViolation {
...
// --- v1 runtime floor: well-formed args => E_NOTHING_TO_REDO --------------

#[test]
fn compute_patch_positive_steps_returns_nothing_to_redo() {
    let prior = empty_project();
    let args = TimelineRedoArgs {
        project_id: fixture_project_id(),
        steps: Some(2),
    ...
    let TimelineRedoError::NothingToRedo { detail } = err else {
        panic!("expected NothingToRedo");
    };
    assert!(detail.contains("requested_steps=2"), "detail: {detail}");
}

#[test]
fn compute_patch_omitted_steps_returns_nothing_to_redo() {
    let prior = empty_project();
...
#[test]
fn error_text_includes_e_nothing_to_redo() {
    let prior = empty_project();
    let verb = TimelineRedoVerb;
    let err = verb
        .compute_patch(
    ...
    assert!(
        detail.contains("E_NOTHING_TO_REDO"),
        "detail should include E_NOTHING_TO_REDO: {detail}",
    );
}

#[test]
fn timeline_redo_error_into_verb_error_custom_for_nothing_to_redo() {
    let err = TimelineRedoError::NothingToRedo {
        detail: "empty redo stack".to_string(),
    };
    let mapped: VerbError = err.into();
...
fn future_success_data_serializes_full_shape() {
    let data = TimelineRedoData {
        redone_event_ids: vec![
...
fn reconstruct_rejects_malformed_args() {
    ...
    assert!(
        msg.contains("TimelineRedoArgs") || msg.contains("wrong type"),
...
fn default_fixture_has_empty_patch_warnings_and_null_expected_data() {
    let fixture = default_fixtures()
        .into_iter()
        .find(|event| event.verb == "timeline.redo")
        .expect("default_fixtures includes timeline.redo");
...
#[test]
fn default_fixture_validates_with_timeline_redo_only_registry() {
    let fixture = default_fixtures()
        .into_iter()
        .find(|event| event.verb == "timeline.redo")
        .expect("default_fixtures includes timeline.redo");
    ...
    registry
        .register(Arc::new(TimelineRedoVerb))
        .expect("register timeline.redo");

    let report = validate_reconstructors(&registry, &[fixture]).expect("validation should pass");
    assert_eq!(report.verbs_checked, vec!["timeline.redo"]);
    assert_eq!(report.fixtures_run, 1);
}

#[test]
fn default_registry_contains_timeline_redo() {
    let registry = default_registry();
    let verb = registry
        .get("timeline.redo")
        .expect("timeline.redo must be in default_registry");
    assert_eq!(verb.verb(), "timeline.redo");
}
...
fn mutate_via_verb_returns_custom_on_v1_floor() {
    ...
    match outcome {
        Err(err) => {
            let msg = err.to_string();
            assert!(
                msg.contains("E_NOTHING_TO_REDO"),
                "error should include E_NOTHING_TO_REDO: {msg}",
            );
        }
