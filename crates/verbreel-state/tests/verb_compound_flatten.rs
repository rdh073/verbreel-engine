//! Tests for `compound.flatten` (§20.3) — v1 compound-schema floor.

use std::sync::Arc;

use serde_json::{Value, json};
use verbreel_state::verbs::compound_flatten::compute_patch;
use verbreel_state::{
    CompoundFlattenArgs, CompoundFlattenData, CompoundFlattenError, CompoundFlattenVerb, Project,
...
fn args_default() -> CompoundFlattenArgs {
    CompoundFlattenArgs {
...
fn args_deserialize_minimal() {
    ...
    let parsed: CompoundFlattenArgs = serde_json::from_value(raw).expect("minimal args parse");
...
fn args_unknown_field_rejected() {
    ...
    let err =
        serde_json::from_value::<CompoundFlattenArgs>(raw).expect_err("unknown field rejects");
...
fn future_data_serializes_exact_fields() {
    let data = CompoundFlattenData {
        removed_compound_clip_id: CLIP_ID_1.parse().expect("clip id"),
        new_clip_ids: vec![
            CLIP_ID_1.parse().expect("clip id"),
            CLIP_ID_2.parse().expect("clip id"),
        ],
        track_id: TRACK_ID.parse().expect("track id"),
        constituent_count: 2,
        freed_compound_asset_id: ASSET_ID.parse().expect("asset id"),
    };
    ...
    assert_eq!(
        ...
        vec![
            "constituent_count",
            "freed_compound_asset_id",
            "new_clip_ids",
            "removed_compound_clip_id",
            "track_id",
        ]
    );
}
...
fn reserved_error_variants_display_codes_and_map_to_custom() {
    let cases = vec![
        ...
        CompoundFlattenError::ClipOverlap {
            failed_clip: CLIP_ID_2.to_string(),
        },
        CompoundFlattenError::Locked {
...
fn reconstruct_rejects_malformed_args() {
    ...
    assert!(err.to_string().contains("CompoundFlattenArgs"));
}
...
fn reconstruct_from_default_fixture_with_single_verb_registry() {
    let fixture = default_fixtures()
        .into_iter()
        .find(|event| event.verb == "compound.flatten")
        .expect("fixture present");

    let mut registry = VerbRegistry::new();
    registry
        .register(Arc::new(CompoundFlattenVerb))
        .expect("register verb");

    let report = validate_reconstructors(&registry, &[fixture]).expect("validate fixture");
    assert_eq!(report.verbs_checked, vec!["compound.flatten"]);
...
#[test]
fn default_registry_contains_compound_flatten() {
    let registry = default_registry();
    assert!(
        registry.get("compound.flatten").is_some(),
        "compound.flatten should be registered"
    );
}
...
fn verb_trait_lookup_via_default_registry_returns_compound_not_a_compound() {
    let registry = default_registry();
    let verb = registry
        .get("compound.flatten")
        .expect("compound.flatten in registry");
    let prior = empty_project();
    let detail = custom_detail(
        verb.compute_patch(&prior, &args_value_default())
