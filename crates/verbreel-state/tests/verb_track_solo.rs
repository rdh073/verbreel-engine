//! Tests for `track.solo` (§4.5) — thirteenth production verb.

use std::sync::Arc;

use serde_json::{Value, json};
use verbreel_state::verbs::track_solo::{
    DEFAULT_SOLO, W_NOOP_CODE, compute_patch, data_envelope_from_post_state,
};
use verbreel_state::{
    MutateOutcome, Project, RecordedEvent, Track, TrackKind, TrackSoloArgs, TrackSoloData,
    TrackSoloError, TrackSoloVerb, Verb, VerbError, VerbRegistry, default_fixtures,
...
fn track(id: &str, kind: TrackKind, name: &str, locked: bool, solo: bool) -> Track {
...
fn patch_solo_value(patch: &Value) -> bool {
    ...
    assert_eq!(arr.len(), 1, "track.solo emits one replace op");
...
fn compute_patch_unsoloed_track_solo_true() {
    ...
    let args = TrackSoloArgs {
        project_id: fixture_project_id(),
        track: TRACK_VIDEO_A.to_string(),
        solo: Some(true),
    };

    let (patch, warnings, data) = compute_patch(&prior, &args).expect("happy-path solo");
...
fn compute_patch_soloed_track_unsolo_false() {
    ...
    let (patch, warnings, data) = compute_patch(&prior, &args).expect("happy-path un-solo");
    assert!(!patch_solo_value(&patch));
    assert!(warnings.is_empty());
    assert_eq!(data.track_id.to_string(), TRACK_VIDEO_A);
    assert!(!data.solo);
...
fn compute_patch_defaulted_solo_none_is_true() {
    ...
    let (patch, warnings, data) = compute_patch(&prior, &args).expect("omitted solo defaults true");
    assert_eq!(patch_solo_value(&patch), DEFAULT_SOLO);
    assert!(warnings.is_empty());
    assert_eq!(data.track_id.to_string(), TRACK_VIDEO_A);
    assert!(data.solo);
}
...
fn compute_patch_idempotent_already_soloed_emits_w_noop() {
    ...
    let (patch, warnings, data) = compute_patch(&prior, &args).expect("same-state solo is a no-op");
    let patch_arr = patch.as_array().expect("patch is array");
    assert!(patch_arr.is_empty(), "same-state solo is a no-op");
    assert_eq!(warnings.len(), 1);
    assert_eq!(warnings[0]["code"], W_NOOP_CODE);
    assert_eq!(warnings[0]["message"], "track solo state unchanged");
    assert_eq!(warnings[0]["details"]["track_id"], TRACK_VIDEO_A);
    assert_eq!(warnings[0]["details"]["solo"], true);
    assert_eq!(data.track_id.to_string(), TRACK_VIDEO_A);
    assert!(data.solo);
}
...
fn compute_patch_idempotent_already_not_soloed_emits_w_noop() {
    ...
    assert_eq!(warnings[0]["message"], "track solo state unchanged");
    assert_eq!(warnings[0]["details"]["track_id"], TRACK_VIDEO_A);
    assert_eq!(warnings[0]["details"]["solo"], false);
    assert_eq!(data.track_id.to_string(), TRACK_VIDEO_A);
    assert!(!data.solo);
}
...
fn compute_patch_locked_track_rejects_with_e_locked() {
    ...
    match err {
        TrackSoloError::Locked { track_id } => assert_eq!(track_id, TRACK_VIDEO_A),
...
fn compute_patch_bad_uuid_errors() {
    ...
    match err {
        TrackSoloError::BadSelector { detail } => {
...
fn compute_patch_track_not_found_errors() {
    ...
    match err {
        TrackSoloError::TrackNotFound { track_id } => assert_eq!(track_id, MISSING_TRACK),
...
fn reconstructor_round_trip() {
    ...
    let post_state = prior
        .apply(&typed_patch)
        .expect("track.solo patch should apply cleanly");
...
fn compute_patch_error_variants_map_to_bad_args() {
    ...
    let verb = TrackSoloVerb;

    let bad_selector = serde_json::to_value(TrackSoloArgs {
        project_id: fixture_project_id(),
        track: "not-a-uuid".to_string(),
        solo: Some(true),
    })
    ...
    let track_not_found = serde_json::to_value(TrackSoloArgs {
        project_id: fixture_project_id(),
        track: MISSING_TRACK.to_string(),
        solo: Some(true),
    })
    ...
    let locked = serde_json::to_value(TrackSoloArgs {
        project_id: fixture_project_id(),
        track: TRACK_VIDEO_A.to_string(),
        solo: Some(false),
    })
...
fn reconstruct_from_default_fixture() {
    let fixture = default_fixtures()
        .into_iter()
        .find(|event| event.verb == "track.solo")
        .expect("default_fixtures includes track.solo");
    ...
    let report = validate_reconstructors(&registry, &[fixture])
        .expect("track.solo reconstruct from fixture");
    assert_eq!(report.verbs_checked, vec!["track.solo"]);
    assert_eq!(report.fixtures_run, 1);
}
...
fn verb_routes_through_mutate_via_verb() {
    ...
    let outcome = store
        .mutate_via_verb("track.solo", args, None)
    ...
    let data: TrackSoloData =
        serde_json::from_value(data).expect("track.solo data is TrackSoloData");
    assert_eq!(data.track_id.to_string(), track_id);
    assert!(data.solo);
    assert!(store.project().tracks[0].solo);
