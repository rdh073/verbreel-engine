//! Tests for `compound.expand` (§20.2) — v1 compound-schema floor.

use std::sync::Arc;

use serde_json::{Value, json};
use verbreel_state::verbs::compound_expand::compute_patch;
use verbreel_state::{
    CompoundExpandArgs, CompoundExpandData, CompoundExpandError, CompoundExpandVerb, Project, Verb,
...
fn args_default() -> CompoundExpandArgs {
    CompoundExpandArgs {
...
fn args_deserialize_minimal() {
    ...
    let parsed: CompoundExpandArgs = serde_json::from_value(raw).expect("minimal args parse");
...
fn args_unknown_field_rejected() {
    ...
    let err = serde_json::from_value::<CompoundExpandArgs>(raw).expect_err("unknown field rejects");
...
fn reconstruct_rejects_malformed_args() {
    ...
    assert!(err.to_string().contains("CompoundExpandArgs"));
}
...
fn reconstruct_from_default_fixture_with_single_verb_registry() {
    let fixture = default_fixtures()
        .into_iter()
        .find(|event| event.verb == "compound.expand")
        .expect("fixture present");

    let mut registry = VerbRegistry::new();
    registry
        .register(Arc::new(CompoundExpandVerb))
        .expect("register verb");

    let report = validate_reconstructors(&registry, &[fixture]).expect("validate fixture");
    assert_eq!(report.verbs_checked, vec!["compound.expand"]);
...
#[test]
fn default_registry_contains_compound_expand() {
    let registry = default_registry();
    assert!(
        registry.get("compound.expand").is_some(),
        "compound.expand should be registered"
    );
}
...
fn verb_trait_lookup_via_default_registry_returns_compound_not_a_compound() {
    ...
    let verb = registry
        .get("compound.expand")
        .expect("compound.expand in registry");
    let prior = empty_project();
    let detail = custom_detail(
        verb.compute_patch(&prior, &args_value_default())
